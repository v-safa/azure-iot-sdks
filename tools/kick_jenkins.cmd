...
if not defined remote (
    ...
    echo Could not isolate remote name for tracking branch ^'%tracking_branch%^'! Aborting...
    goto :eof
)
...
REM find repo url
for /f "usebackq tokens=*" %%i in (`git ls-remote --get-url %remote%`) do set "repo_url=%%i" 
...
echo remote:    %remote%
echo trk_abbrv: %tracking_abbrev%
echo ***********************************


REM kick off the build!
rem java -jar "%build-root%"\tools\jenkins-cli.jar -s http://azure-iot-sdks-ci.westus.cloudapp.azure.com:8080/ build _anyrepo2M-integrate-into-develop -p COMMIT_ID=%current_branch% -p AZURE_REPO=%repo_url% -p BRANCH_TO_MERGE_TO=develop -s -v
